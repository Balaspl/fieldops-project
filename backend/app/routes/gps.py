...
def check_sliding_window_rate_limit(
    ...
    try:
        ...
        # Add the current request timestamp with a unique identifier
        ...
        logger.warning(
            f"Redis rate limiter connection issue: {e}. "
            f"Falling back to allowing request."
        )
        return True
...
async def gps_ping(
    ...
    # (runs before database session is acquired)
    try:
    ...
    # Acquire database session only AFTER validation passes
    db_dep = request.app.dependency_overrides.get(
        get_db,
        get_db
    )
    ...
    try:
        ...
        if str(payload.job_id).isdigit():
            job = db.query(models.Job).filter(
                models.Job.id == int(payload.job_id),
        ...
        if not is_legacy_active:
            ...
            job_status_upper = (
                job.status.upper().strip()
            )

            if job_status_upper not in active_statuses:
                logger.error(
                    f"Job status outside active window: "
                    f"{job.status}",
                    extra=log_extra
                )
            ...
            if (
                ...
                logger.error(
                    f"Technician {payload.technician_id} "
                    f"not assigned to job {payload.job_id}",
                    extra=log_extra
                )
            ...
            if job.status.lower() != "active":
                logger.error(
                    f"Job status not active: {job.status}",
                    extra=log_extra
                )
        ...
        if not bypass_interval and not is_legacy_active:
            # Check and handle job status transition
            # to reset interval key
            current_status = job_status_upper
            status_key = (
                f"gps:job_status:{x_tenant_id}:{payload.job_id}"
            )

            interval_key = (
                f"gps:interval:{x_tenant_id}:"
                f"{payload.technician_id}:{payload.job_id}"
            )
            ...
            if not allowed:
                if mode == "redis":
                    reason = (
                        "GPS ping interval minimum 30 seconds"
                    )
        ...
        # max 1 ping per 30 seconds per technician
        rate_limit_allowed = check_sliding_window_rate_limit(
            redis_client,
            payload.technician_id,
            x_tenant_id
        )
        ...
        if not rate_limit_allowed:
            logger.warning(
                f"Rate limit exceeded for technician: "
                f"{payload.technician_id}",
                extra=log_extra
            )
        ...
        # Insert into database with tenant_id isolation
        # from technician record
        ...
        # Capture ping attributes before commit to avoid
        # ObjectDeletedError after the after_insert event
        ...
        # Refresh job right before commit
        db.refresh(job)

        if job.status.upper().strip() in [
            "CLOSED",
            "CANCELLED",
            "CANCELED"
        ]:
            ...
            return JSONResponse(
                status_code=status.HTTP_409_CONFLICT,
                content={
                    "detail": "Job status changed during processing",
                    "status": 409
                }
            )
        ...
        try:
            update_payload = {
                ...
                "accuracy": (
                    float(ping_accuracy)
                    if ping_accuracy is not None
                    else None
                ),
                "altitude": (
                    float(ping_altitude)
                    if ping_altitude is not None
                    else None
                ),
                "job_status": (
                    job.status
                ...
                "timestamp": (
                    ping_timestamp.isoformat()
                    if hasattr(ping_timestamp, "isoformat")
                    else str(ping_timestamp)
                ),
                "broadcast_at": (
            ...
            try:
                eta_key = (
                    f"eta:{ping_technician_id}:{ping_job_id}"
                )
                ...
                if eta_raw:
                    ...
                    update_payload["eta"] = eta_data.get(
                        "eta",
                        "calculating..."
                    )
                    update_payload[
                        "eta_duration_minutes"
                    ] = eta_data.get("duration_minutes")
            ...
            logger.warning(
                f"Failed to publish GPS ping to Redis: {e}"
            )
...
def check_batch_sliding_window_rate_limit(
    ...
    Enforces max 1 batch request per 5 seconds per technician
    using a sliding window in Redis.
    """
    global redis_failures_count
    ...
    key = (
        f"rate_limit:gps_batch:{tenant_id}:{technician_id}"
    )
    ...
    try:
        ...
        logger.warning(
            f"Redis rate limiter connection issue for batch: "
            f"{e}. Falling back to allowing request."
        )
        return True
...
async def gps_batch(
    ...
    if not isinstance(body, dict) or "pings" not in body:
        return JSONResponse(
            status_code=status.HTTP_400_BAD_REQUEST,
            content={
                "detail": "Pings array cannot be empty"
            }
        )
    ...
    if len(pings) > 100:
        return JSONResponse(
            status_code=413,
            content={
                "detail": "Maximum 100 pings per batch"
            }
        )

    # Validate with Pydantic GPSBatchRequest schema
    try:
    ...
    logger.info(
        f"Received GPS batch insert request with "
        f"{len(pings)} pings",
        extra=log_extra
    )
    ...
    for tech_id in tech_ids:
        if tech_id:
            rate_limit_allowed = (
                check_batch_sliding_window_rate_limit(
                    redis_client,
                    tech_id,
                    x_tenant_id
                )
    ...
    # Phase 1: Coordinate range, type, null/missing,
    # and duplicate timestamp validation
    for i, ping in enumerate(pings):
        if not isinstance(ping, dict):
            errors.append({
                "index": i,
                "reason": "Ping must be a JSON object"
            })
            continue
        ...
        if key in seen_timestamps:
            errors.append({
                "index": i,
                "reason": "Duplicate timestamp within batch"
            })
            continue
    ...
    try:
        # Phase 2: Technician and Job existence validation
        ...
        job_records = db.query(
            models.Job
        ).filter(
            models.Job.id.in_(numeric_job_ids),
            models.Job.tenant_id == x_tenant_id,
        ).all()
        ...
        for i, payload in validated_pings:
            ...
            if (
                ...
                errors.append({
                    "index": i,
                    "reason": "Access denied for technician"
                })
            ...
            if not is_legacy_active:
                ...
                if (
                    ...
                    errors.append({
                        "index": i,
                        "reason": "Job status is not active"
                    })
                ...
                if (
                    ...
                    errors.append({
                        "index": i,
                        "reason": "Technician not assigned to job"
                    })
        ...
        if errors:
            ...
            logger.warning(
                f"GPS batch insert failed validation "
                f"with {failed} errors. Rolling back.",
                extra=log_extra
            )
        ...
        # Race Condition Prevention:
        # Refresh all jobs right before execute/commit
        for j in job_records:
            db.refresh(j)

            if j.status.upper().strip() in [
                "CLOSED",
                "CANCELLED",
                "CANCELED"
            ]:
                ...
                # Log rejected pings for all pings in this batch
        ...
        if insert_dicts:
            ...
            try:
                ...
                for d in insert_dicts:
                    ...
                    try:
                        eta_key = (
                            f"eta:{d['technician_id']}:"
                            f"{d['job_id']}"
                        )

                        eta_raw = redis_client.get(
                            eta_key
                        )
                ...
                logger.warning(
                    f"Failed to publish GPS batch to Redis: {e}"
                )
        ...
        logger.info(
            f"Successfully processed GPS batch insert "
            f"for {len(insert_dicts)} pings",
            extra=log_extra
        )
