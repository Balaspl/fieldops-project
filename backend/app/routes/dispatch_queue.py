...
def calculate_sla_risk(
    ...
    remaining_minutes = (
        (deadline - now_utc).total_seconds() / 60
    )
...
def get_dispatch_queue(
    ...
    if zone:
        query = query.filter(
            Job.location.ilike(f"%{zone}%")
        )

    # Apply cursor pagination if present
    # Using sla_deadline ASC (nulls last) as primary sort,
    # and id ASC as secondary
    if cursor:
        try:
            ...
            if c_date_str != "null":
                ...
                # If the previous page's last item had no deadline,
                # only look at items with no deadline and id > c_id
                query = query.filter(
    ...
    for job in results:
        minutes_remaining, risk_level = calculate_sla_risk(
            job.sla_deadline
        )
        ...
        if job.technician:
            tech_detail = QueueTechnicianDetail(
                tech_id=(
                    ...
                    or str(job.technician.technician_id)
                ),
