...
def get_dispatch_queue(
    status: Optional[str] = Query(
        None,
        description="Filter by status (e.g., QUEUED, ASSIGNED)",
    ),
    ...
    zone: Optional[str] = Query(
        None,
        description="Filter by zone (matches location)",
    ),
    ...
    # Tenant is derived from the authenticated user.
    ...
    # Using sla_deadline ASC (nulls last)
    # as primary sort, and id ASC as secondary.
    if cursor:
        try:
            cursor_str = (
            ...
            if c_date_str != "null":
                ...
                # If the previous page's last item
                # had no deadline, only look at
                # items with no deadline and id > c_id.
                query = query.filter(
    ...
    # Sort:
    # highest SLA risk first means earliest deadline first.
    ...
    for job in results:
        ...
        if job.technician:
            tech_detail = QueueTechnicianDetail(
                tech_id=(
                    job.technician.tech_id
                    or str(
                        job.technician.technician_id
                    )
                ),
                name=(
                    job.technician.technician_name
                ),
                status=(
                    job.technician.technician_status
                ),
            )
    ...
    if has_more and results:
        ...
        c_date_str = (
            last_item.sla_deadline.isoformat()
            if last_item.sla_deadline
            else "null"
        )

        cursor_str = (
            f"{c_date_str}|{last_item.id}"
        )
    ...
    # Log metrics to ensure NFR-001
    # (P95 < 200ms) is visible.
    import logging

    logger = logging.getLogger(__name__)

    logger.info(
        "dispatch_queue_accessed - "
        f"{len(results)} items in "
