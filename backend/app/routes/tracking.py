...
async def ws_tracking(
    ...
    Technicians are restricted to their own technician/job
    tracking resources and tenant.
    """
    ...
    try:
        while True:
            ...
            #
            # IMPORTANT:
            # This also passes tenant_id so tracking_manager can apply the
            # same tenant/ownership authorization used for subscriptions.
            # -----------------------------------------------------------------
            ...
            # -----------------------------------------------------------------
                ...
                # Heartbeat pong is handled by the heartbeat coroutine.
                # Messages arriving here outside the heartbeat wait window
                # are ignored.
                pass
    ...
    try:
        ...
        async for message in pubsub.listen():
            ...
            try:
                ...
                logger.error(
                    f"[ws:listener] Error processing GPS message: {e}"
                )

    except asyncio.CancelledError:
        logger.info(
            "[ws:listener] Redis GPS listener cancelled"
        )

    except Exception as e:
        logger.error(
            f"[ws:listener] Fatal error in GPS listener: {e}",
            exc_info=True,
        )
