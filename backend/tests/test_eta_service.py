...
def test_eta_calculation_with_valid_gps_and_job_site(setup_db):
    ...
    with patch(
        ...
        return_value=MockResponse(mock_maps_response, 200),
    ):
        response = client.get(
        ...
        assert data["traffic_delay_minutes"] == 7.0

        eta_time = datetime.fromisoformat(
            data["eta"].replace("Z", "+00:00")
        )
...
def test_redis_cache_stores_eta_for_30_seconds(setup_db):
    ...
    with patch(
        "aiohttp.ClientSession.get",
        side_effect=Exception("API called"),
    ):
        response = client.get(
            "/api/v1/eta?technician_id=tech-123&job_id=101",
            headers=build_headers(tenant_id="tenant-1"),
...
def test_batch_eta_calculation_for_3_technicians(setup_db):
    ...
    with patch(
        "aiohttp.ClientSession.get",
        return_value=MockResponse(mock_batch_response, 200),
    ):
        payload = {
            "technician_ids": [
                "tech-1",
...
def test_multi_tenant_isolation(setup_db):
    ...
    # Request is authenticated as tenant-1.
    # The job belongs to tenant-2, so access must be denied.
    response = client.get(
        "/api/v1/eta?technician_id=tech-123&job_id=101",
        headers=build_headers(tenant_id="tenant-1"),
    )
